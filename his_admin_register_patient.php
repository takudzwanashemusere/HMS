...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['add_patient']))
		{
            ...
            $check_query = "SELECT * FROM his_patients WHERE pat_fname = ? AND pat_lname = ?";
            $stmt = $mysqli->prepare($check_query);
            $stmt->bind_param('ss', $pat_fname, $pat_lname);
            $stmt->execute();
            $result = $stmt->get_result();
        
            if ($result->num_rows > 0) {
                // Duplicate found
                $err = "A patient with the same name and surname already exists.";
            } else {
                // Proceed with inserting new patient
                $query = "INSERT INTO his_patients (pat_fname, pat_ailment, pat_lname, pat_age, pat_dob, pat_number, pat_phone, pat_type, pat_addr) 
                          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $mysqli->prepare($query);
                $stmt->bind_param('sssssssss', $pat_fname, $pat_ailment, $pat_lname, $pat_age, $pat_dob, $pat_number, $pat_phone, $pat_type, $pat_add);
